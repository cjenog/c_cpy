...
void	ft_change(long lnbr, int base_len, char *base) // 넘겨받은 long형 nbr을 이제 base_to기준으로 생김새를 변환해주는 함수.재귀적으로 구성되있음.
{
	if (lnbr >= base_len)
	{	//printf("%ld, %d, %s 입니다.\n",lnbr,base_len,base);
		ft_change(lnbr / base_len, base_len, base);
...
void ft_convert_base(char* nbr, char* base_from, char* base_to)
{
	...
	//printf("lnbr : %ld \n", lnbr);
	ft_change(lnbr,base_to_len,base_to);  // 10진수를 base_to기준으로 기수화시켜줌.  
	//return nbr;
